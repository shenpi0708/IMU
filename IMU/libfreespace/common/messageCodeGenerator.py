            ...
            for field in message.Fields[v]:
                ...
                if 'bits' in field:
                    ...
                    if field['typeDecode']['count'] == 1:
                        for j in np.arange(field['typeDecode']['width']):
                            outFile.write('\t\t\tmessage[%d + offset] = s->%s >> %d;\n'%(byteCounter, field['name'], 8 * j))
                            byteCounter += 1
                    else:
                        for i in np.arange(field['typeDecode']['count']):
                            for j in np.arange(field['typeDecode']['width']):
...
'''%{'size':4, 'id':message.ID[v]['constID'], 'name':message.name})
