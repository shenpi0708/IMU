...
static void getEulerAnglesFromMotion(const struct freespace_MotionEngineOutput* meOut,
                                     struct MultiAxisSensor* quaternion);
...
int main(int argc,char **argv)
{
    ...
    while(!quit)
    {
        ...
        if (rc) {
            int i;
            // Run game logic.
            getEulerAnglesFromMotion(&meOut, &quaternion);
...
//            }
            ...
            //printf("%f\n",accel.y);
            //printf("acc_mag:%f\n",acc_mag);
            //printf("roll: %0.4f, pitch: %0.4f, yaw: %0.4f\n",RADIANS_TO_DEGREES(quaternion.x),RADIANS_TO_DEGREES(quaternion.y),RADIANS_TO_DEGREES(quaternion.z));
        ...
        //ROS publish msg
        sensor_msgs::Imu inertia2;
        inertia2.header.frame_id='map';
        inertia2.orientation.x = quaternion.x;
        inertia2.orientation.y = quaternion.y;
        inertia2.orientation.z = quaternion.z;
        inertia2.orientation.w = quaternion.w;
        inertia2.angular_velocity.x=angularVel.x;
        inertia2.angular_velocity.y=angularVel.y;
        inertia2.angular_velocity.z=angularVel.z;
        inertia2.linear_acceleration.x=accel.x;
        inertia2.linear_acceleration.y=accel.y;
        inertia2.linear_acceleration.z=accel.z;
        //printf("%f\t%f\n",position(0),position(1));

        imu_pub.publish(inertia2);
 ...
 */
static void getEulerAnglesFromMotion(const struct freespace_MotionEngineOutput* meOut,struct MultiAxisSensor* q)
{
    struct MultiAxisSensor sensor;
    //struct Vec3f quaternion;
...
static void* inputThreadFunction(void* arg)
{
    ...
    while (!state->quit_) {
        ...
        if (message.messageType == FREESPACE_MESSAGE_MOTIONENGINEOUTPUT) {
            ...
            state->updated_ = 1;
            //printf("%s\n",typeid(message.motionEngineOutput).name());
